  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    const { data, error: insertError } = await supabase
      ...
      .insert({
        user_id: user.id,
        ticker,
        company_name: ticker, // Will be updated when we have profile data
        sector: null,
      })
      .select()
      .single()
    ...
    setRows((prev) => [...prev, data as WatchlistRow])
    setNewTicker('')
    setAdding(false)
  }
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="text-right px-4 py-3 text-xs font-mono text-text-muted uppercase tracking-widest hidden sm:table-cell">Sector</th>
                  <th className="text-right px-4 py-3 text-xs font-mono text-text-muted uppercase tracking-widest">Added</th>
